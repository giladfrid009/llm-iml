...
# ============================== UTILS FOR UAT ============================== #
def hotflip_attack(averaged_grad: torch.Tensor, embedding_matrix: torch.Tensor, trigger_token_ids, increase_loss:bool=False, num_candidates:int=1) -> np.ndarray | int:
    """
    ...
    trigger_token_embeds = torch.nn.functional.embedding(torch.LongTensor(trigger_token_ids).cuda(), embedding_matrix).detach().unsqueeze(0)
...
class UAT(SingleBehaviorRedTeamingMethod):
    ...
    def generate_test_cases_single_behavior(self, behavior_dict, num_generate, verbose=False) -> str:
        """
        ...
        embedding_weight: torch.Tensor = embed_layer.weight.detach()
        ...
        if self.use_prefix_cache:
            ...
            input_embeds = torch.cat(
                [before_embeds, behavior_embeds, embed_layer(trigger_tokens.unsqueeze(0)), after_embeds, target_embeds], dim=1
            )
            ...
            for token_to_flip in range(0, trigger_token_length):
                ...
                if self.use_prefix_cache:
                    input_embeds_candidates = torch.cat(
                        [candidates_embeds, after_embeds.repeat(len(candidates), 1, 1), target_embeds.repeat(len(candidates), 1, 1)], dim=1
                    )
                else:
                    input_embeds_candidates = torch.cat(
                        [
